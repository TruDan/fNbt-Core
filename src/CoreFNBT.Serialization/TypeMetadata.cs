            ...
            if (Category == TypeCategory.ConvertibleByProperties) {
                Properties =
                    contractType.GetProperties()
                                .Where(p => !p.GetCustomAttributes(typeof(NbtIgnoreAttribute)).Any())
                                .ToArray();
                PropertyTagNames = new Dictionary<PropertyInfo, string>();

                foreach (PropertyInfo property in Properties) {
                    // read [TagName] attributes
	                Attribute[] nameAttributes = property.GetCustomAttributes(typeof(TagNameAttribute)).ToArray();
	                ...
	                var nullPolicyAttr = property.GetCustomAttribute<NullPolicyAttribute>();
                    ...
                    if (property.IsDefined(typeof(NonSerializedAttribute)) ||
                        property.IsDefined(typeof(NbtIgnoreAttribute))) {
